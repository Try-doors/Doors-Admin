...
export function DetailPanel({
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end bg-black/40" onClick={onClose}>
      ...
      >
        <div className="flex items-center justify-between px-5 pt-6 pb-[26px] sm:px-[30px]">
          <p className="text-[20px] font-semibold tracking-[-0.5px] text-[#0C111D]">{title}</p>
          {headerRight}
        </div>
        <div className="flex-1 overflow-y-auto px-5 sm:px-[30px]">{children}</div>
        <div className="flex h-[88px] shrink-0 items-center px-5 sm:px-[30px]">
          <button
            type="button"
            onClick={onClose}
